...
async def sincronizar_rangos(guild):
    ...
    permisos_rangos = cfg.get("permisos_rangos", {})
    ...
    for nombre in jerarquia:
        ...
        if nombre not in existentes:
            try:
                ...
                Logger.info(f"❌ No se pudo crear {nombre}: {e}")
        else:
            ...
            if cambios:
                try:
                    ...
                    Logger.info(f"⚠️ No se pudo actualizar {nombre}: {e}")
    ...
    try:
        ...
        Logger.info(f"⚠️ No se pudo ordenar: {e}")
